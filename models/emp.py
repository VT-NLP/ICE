...
class SoftPrompt(nn.Module):
    def __init__(self, plm_embed: nn.Embedding, label_mapping: dict, n_prompt: int = PROMPT_SIZE, random_range: float = 0.1,
        ...
        #     self.prompt_na = nn.parameter.Parameter(self.event_tensor_list[0], requires_grad=False)
        # else:
        #     self.prompt_na = nn.parameter.Parameter(self.event_tensor_list[0], requires_grad=True)
        # pdb.set_trace()
        self.prompt_list = nn.ParameterList([nn.parameter.Parameter(self.event_tensor_list[i], requires_grad=False) for i in range(len(self.event_tensor_list))])
        self.prompt_list[0].requires_grad = True
        self.prompt_list[1].requires_grad = True
        self.prompt_list[-1].requires_grad = True
    ...
    def build_event_repr(self, random_range):
        ...
        for i in range(len(id2tokens)):
            ...
            if opts.task_type == "ec" and i == 0:       # add dummy prompt
    ...
    def forward(self, tokens):
        # pdb.set_trace()
        prompt_embed = torch.cat([self.prompt_list[i] for i in range(TASK_NUM+2)], dim=0)
...
class PromptNet(MetaModule):
    def __init__(self, input_dim: int, hidden_dim: int, max_slots: int, init_slots: int, label_mapping: dict,
        ...
        self.input_map = MetaSequential(OrderedDict({
            "linear_0": MetaLinear(input_dim*k, hidden_dim*k),
            "relu_0": nn.ReLU(),
            "dropout_0": nn.Dropout(0.2),
            "linear_1": MetaLinear(hidden_dim*k, hidden_dim*k),
            "relu_1": nn.ReLU(),
            "dropout_1": nn.Dropout(0.2),
            "linear_2": MetaLinear(hidden_dim*k, hidden_dim*k),
            "relu_2": nn.ReLU()
        }))
        self.prompt_map = MetaSequential(OrderedDict({
            "linear_0": MetaLinear(self.half_input_dim, hidden_dim*k),
            "relu_0": nn.ReLU(),
            "linear_1": MetaLinear(hidden_dim*k, hidden_dim*k),
            "relu_1": nn.ReLU(),
            "linear_2": MetaLinear(hidden_dim*k, hidden_dim*k),
        ...
        self.classes = MetaLinear(hidden_dim*k, max_slots, bias=False)
