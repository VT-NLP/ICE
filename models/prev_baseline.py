...
class BERT(MetaModule):
    def __init__(self, input_dim: int, hidden_dim: int, max_slots: int, init_slots: int, label_mapping: dict,
        ...
        # self.bert.resize_token_embeddings(BERT_VOCAB_SIZE + BERT_MAXLEN)
        ...
        if opts.balance == "ffe":
            self.classes = MetaLinear(hidden_dim*k*2, max_slots, bias=False)
        else:
    ...
    def forward(self, batch, nslots: int = -1, exemplar: bool = False, exemplar_distill: bool = False,
        ...
        scores = self.classes(cls_in, params=self.get_subdict(params, "classes"))
        ...
        if log_outputs:
            ...
            if opts.case_study and curr_stream:
                ...
                # if torch.any((labels < max_id) & (labels >= self.new_start_id)):
    ...
    def replay_forward(self, params, task_id, idx):
        features = self.exemplar_features[idx].to(self.device).unsqueeze(0)
        inputs = self.input_map(features, params=self.get_subdict(params, "input_map"))
            ...
            for i in label_sets:
                ...
                if i == 0:
                    ...
                    try:
                        ...
                        print("cannot sample for none type")
...
def test():  # sanity check
    m = BERT(nhead=8, nlayers=3, hidden_dim=512, input_dim=2048, max_slots=30, init_slots=9,
