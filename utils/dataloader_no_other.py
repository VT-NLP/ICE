...
class DataCollection(object):
    ...
    def __init__(self, root:str, feature_root:str, stream:List[List[Union[str,int]]], splits:Union[List[str],None]=None) -> None:
        ...
        self.splits_span_cnt = {"train": 0, "dev": 0, "test": 0} 
                ...
                for split in data:
                    for t in data[split]:
                        ...
                        for i in range(len(t['label'])):
                            ...
                            # ------- introduce negative type in training
                            elif split == "train" and opts.ec_train_other:
                                add_label.append(0)                # TODO (neg): add the span in the future tasks as Other
                                add_span.append(t['span'][i])
                                add_idx.append(idx)
                                idx += 1
                        if len(add_label) != 0 and add_flag:
                            ...
                            new_t['span'] = add_span
                            new_t['batch_id'] = add_idx
                            response[split].append(new_t)
                    self.splits_span_cnt[split] = idx
...
class Instance(object):
    ...
    def __init__(self, token_ids:List[int], label:List[int], span:List[Tuple[int, int]], features:Union[str, torch.FloatTensor], sentence_id:str, mention_id:List[str], batch_id:int) -> None:
...
class LabelDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Instance:
        ...
        return instance.load_clone(index)
...
def get_stage_loaders(root:str,
    ...
    **kwargs):
    ...
    for stream in streams:
        ...
        # TODO (neg): make a dev/test loader for each task
        ...
        dev_loader_per_task = DataLoader(
            dataset=dataset_dev,
            batch_size=batch_size,
            shuffle=False,
            drop_last=False,
        ...
        test_loader_per_task = DataLoader(
        ...
        dev_loaders.append(dev_loader_per_task)
        test_loaders.append(test_loader_per_task)


    stages = [[collection.label2id[t] for t in stream] for stream in streams]

    return loaders, dev_loaders, test_loaders, exclude_none_loaders, stages, collection.label2id, collection.splits_span_cnt
'''
