...
class Worker(object):
    ...
    def __init__(self, opts):
        ...
        self.log = opts.log
        self.grad_acc_step = opts.grad_accumulate_step
    ...
    def run_one_epoch(self, model, loader, split, f_loss=None, optimizer=None, scheduler=None, collect_outputs=None, collect_stats=None, **kwargs):
        ...
        # for it, batch in enumerate(loader):
            ...
            if split == "train":
                loss = f_loss(batch)
                loss = loss / self.grad_acc_step
                loss.backward()
                if (it + 1) % self.grad_acc_step == 0:
                    optimizer.step()
                    if scheduler:
                        scheduler.step()
                    model.zero_grad()
                ...
                with torch.no_grad():
                    loss = f_loss(batch)
            if loss > 0:
                epoch_loss += (loss.item()) * self.grad_acc_step
